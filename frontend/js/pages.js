...
function LoginPage({ onLogin }) {
 ...
 React.createElement('div', null,
 React.createElement('h1', { className: 'text-xl font-bold text-slate-800 ' }, 'AutoLeave AI'),
 React.createElement('p', { className: 'text-slate-400 text-xs' }, 'Smart Leave System')
 )
 ),

 React.createElement('div', { className: 'mb-7' },
 React.createElement('h2', { className: 'text-2xl font-bold text-slate-800 mb-1 tracking-tight' }, 'Welcome back'),
 React.createElement('p', { className: 'text-slate-500 text-sm' }, 'Select role and login to see your dashboard.')
 ),
 ...
 React.createElement('p', { className: 'mt-8 text-center text-xs text-slate-400 ' },
...
function DashboardPage({ onNavigate }) {
 ...
 loading
 ? React.createElement(Card, { className: 'p-2 divide-y divide-slate-100 ' },
 ...
 React.createElement('p', { className: 'text-sm text-slate-400 ' }, 'No notifications yet')
 )
 )
 ),
 ),
...
// DECISION MODAL
// ============================================================
function DecisionModal({ isOpen, onClose, onConfirm, title, subtitle, confirmLabel, confirmVariant = 'primary' }) {
 ...
 React.createElement('div', { className: 'relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden' },
 React.createElement('div', { className: 'px-6 py-4 border-b border-slate-100' },
 ...
 React.createElement('label', { className: 'block text-sm font-medium text-slate-700 mb-2' }, 'Reason (optional)'),
 ...
 React.createElement('div', { className: 'px-6 py-4 bg-slate-50 flex justify-end gap-2' },
 React.createElement(Button, { variant: 'secondary', onClick: onClose }, 'Cancel'),
 React.createElement(Button, { variant: confirmVariant, onClick: handleConfirm, loading: isSubmitting }, confirmLabel)
 )
 )
 );
}
...
function FacultyDashboardPage({ loading, data, onNavigate }) {
 ...
 const [modal, setModal] = useState({ open: false, action: null, leaveId: null, title: '', subtitle: '', confirmLabel: '', variant: 'primary' });
 const { addToast } = useToast();
 ...
 subtitle: 'Please provide a reason for rejecting this leave (optional).',
 confirmLabel: 'Reject',
 variant: 'danger'
 });
 };
 ...
 subtitle: 'Please provide a reason for forwarding this leave (optional).',
 ...
 await window.LeaveService.facultyDecision(modal.leaveId, 'rejected', reason);
 addToast('Leave rejected successfully', 'success');
 } else if (modal.action === 'forward') {
 await window.LeaveService.facultyForwardToAdmin(modal.leaveId, reason);
 ...
 try {
 await window.LeaveService.facultyDecision(id, status, '');
 ...
 React.createElement('div', null,
 React.createElement('h2', { className: 'text-xl font-bold text-slate-800 ' }, 'Faculty Dashboard'),
 React.createElement('p', { className: 'text-sm text-slate-500 mt-0.5' }, 'Review pending leaves with AI risk insights.')
 ),
 ...
 React.createElement('h3', { className: 'text-sm font-semibold text-slate-700 mb-3' }, 'Pending Leaves'),
 pendingLoading
 ...
 React.createElement('div', { className: 'flex flex-col sm:flex-row items-start sm:items-center gap-3' },
 ...
 React.createElement(Button, { variant: 'danger', onClick: () => openRejectModal(l.leaveId) }, 'Reject'),
 React.createElement(Button, { variant: 'secondary', onClick: () => openForwardModal(l.leaveId) }, 'Forward to Admin')
 )
 )
 )
 )
 ...
 React.createElement(DecisionModal, {
 isOpen: modal.open,
 onClose: closeModal,
...
function FacultyApplyLeavePage({ onNavigate }) {
 ...
 React.createElement('div', { className: 'flex items-center justify-between' },
 React.createElement('div', null,
 React.createElement('h2', { className: 'text-xl font-bold text-slate-800 ' }, 'Faculty Leave Application'),
 React.createElement('p', { className: 'text-sm text-slate-500 mt-0.5' }, 'Submit your leave request to admin from this dedicated page.')
 ),
...
function AdminDashboardPage({ loading, data, initialTab = 'analytics' }) {
 ...
 React.createElement('div', { className: 'flex items-center justify-between' },
 React.createElement('div', null,
 React.createElement('h2', { className: 'text-xl font-bold text-slate-800 ' }, 'Admin Dashboard'),
 React.createElement('p', { className: 'text-sm text-slate-500 mt-0.5' }, 'Monitor users, leaves, and analytics.')
 ),
...
function ApplyLeavePage({ onNavigate }) {
 ...
 try {
  ...
  studentId: session?.userId || window.AuthService.getSession()?.userId,
 ...
 React.createElement('div', { className: 'w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4' },
 React.createElement(Icon, { name: 'CheckCircle', size: 36, className: 'text-green-500' })
 ),
 React.createElement('h3', { className: 'text-xl font-bold text-slate-800 mb-2' }, 'Application Submitted!'),
 React.createElement('p', { className: 'text-slate-500 text-sm mb-4' }, 'Redirecting to leave history...'),
 ...
 React.createElement('h2', { className: 'text-xl font-bold text-slate-800 mb-1' }, 'Apply for Leave'),
 React.createElement('p', { className: 'text-sm text-slate-500 ' }, 'Fill in the details below to submit your leave request.')
 ),
 ...
 // Leave Details
 React.createElement(Card, { className: 'p-5' },
 React.createElement('h3', { className: 'text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2' },
 ...
 React.createElement('h3', { className: 'text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2' },
 ...
 React.createElement('p', { className: 'text-xs font-semibold text-slate-600 mb-2.5' }, 'Prediction Factors'),
 React.createElement('div', { className: 'space-y-2' },
 (prediction.factors || []).map((f, i) =>
 React.createElement('div', { key: i, className: 'flex items-center justify-between text-xs' },
 React.createElement('span', { className: 'text-slate-600 ' }, f.label),
 React.createElement('span', { className: cn('font-medium flex items-center gap-1', f.impact === 'positive' ? 'text-green-600 ' : f.impact === 'negative' ? 'text-red-500' : 'text-slate-500 ') },
 ...
 React.createElement(Icon, { name: 'Sparkles', size: 24, className: 'text-slate-300 mx-auto mb-2' }),
 React.createElement('p', { className: 'text-sm text-slate-400 leading-relaxed' }, 'Fill in the leave type and reason to see AI prediction.')
 ),

 // Info card
 React.createElement(Card, { className: 'mt-4 p-4 bg-teal-50 border-teal-200 ' },
 React.createElement('div', { className: 'flex gap-2.5' },
 React.createElement(Icon, { name: 'Info', size: 16, className: 'text-teal-600 mt-0.5 flex-shrink-0' }),
 React.createElement('div', null,
 React.createElement('p', { className: 'text-xs font-medium text-teal-700 mb-1' }, 'Before you submit'),
 React.createElement('ul', { className: 'text-xs text-teal-600 space-y-1 list-disc list-inside' },
 [
...
function LeaveHistoryPage({ onNavigate }) {
 ...
 React.createElement('div', { className: 'flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3' },
 React.createElement('div', null,
 React.createElement('h2', { className: 'text-xl font-bold text-slate-800 ' }, 'Leave History'),
 React.createElement('p', { className: 'text-sm text-slate-500 mt-0.5' }, 'All your leave requests and their current status.')
 ),
 ...
 React.createElement('p', { className: 'text-xs text-slate-500 ' }, s.label)
 )
 )
 ),
 ...
 className: 'w-full pl-9 pr-3.5 py-2.5 text-sm rounded-xl border border-slate-200 bg-white text-slate-800 placeholder-slate-400 smooth-transition focus:border-teal-400',
 ...
 className: 'pl-3.5 pr-8 py-2.5 text-sm rounded-xl border border-slate-200 bg-white text-slate-800 smooth-transition focus:border-teal-400 cursor-pointer w-full sm:w-auto',
 ...
 // View toggle
 React.createElement('div', { className: 'flex rounded-xl border border-slate-200 overflow-hidden flex-shrink-0' },
 React.createElement('button', {
 onClick: () => setViewMode('cards'),
 className: cn('p-2.5 smooth-transition', viewMode === 'cards' ? 'bg-teal-50 text-teal-600 ' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50 '),
 'aria-label': 'Card view', 'aria-pressed': viewMode === 'cards',
 }, React.createElement(Icon, { name: 'LayoutGrid', size: 16 })),
 React.createElement('button', {
 onClick: () => setViewMode('table'),
 className: cn('p-2.5 smooth-transition', viewMode === 'table' ? 'bg-teal-50 text-teal-600 ' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50 '),
 ...
 React.createElement('thead', null,
 React.createElement('tr', { className: 'bg-slate-50 text-left' },
 ['ID', 'Type', 'Period', 'Days', 'Status', 'AI Pred.', 'Applied On'].map(h =>
 React.createElement('th', { key: h, className: 'px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap' }, h)
 )
 )
 ),
 ...
 React.createElement('td', { className: 'px-4 py-3 text-slate-500 font-mono text-xs' }, leave.id),
 React.createElement('td', { className: 'px-4 py-3' },
 React.createElement('div', null,
 React.createElement('p', { className: 'font-medium text-slate-700 ' }, leave.type),
 React.createElement('p', { className: 'text-xs text-slate-400 line-clamp-1 max-w-xs' }, leave.reason)
 )
 ),
 React.createElement('td', { className: 'px-4 py-3 text-slate-500 whitespace-nowrap' },
 `${formatDate(leave.startDate)} – ${formatDate(leave.endDate)}`
 ),
 React.createElement('td', { className: 'px-4 py-3 text-slate-700 font-medium' }, `${leave.days}d`),
 React.createElement('td', { className: 'px-4 py-3' }, React.createElement(StatusBadge, { status: leave.status })),
 React.createElement('td', { className: 'px-4 py-3' },
 React.createElement('span', { className: 'text-xs font-medium text-purple-600 ' }, `${leave.aiPrediction}%`)
 ),
 React.createElement('td', { className: 'px-4 py-3 text-slate-500 whitespace-nowrap' }, formatDate(leave.appliedDate))
 )
 )
 )
 )
...
function NotificationsPage({ onNavigate }) {
 ...
 try {
 setLoading(true);
 await window.NotificationService.clearRecentNotifications();
 setNotifications([]);
 addToast('All notifications cleared.', 'success');
 } catch (err) {
 // Even if API fails, clear local state for better UX
 setNotifications([]);
 addToast('Notifications cleared locally.', 'info');
 } finally {
 setLoading(false);
 }
 };
 ...
 notifications.length > 0 && React.createElement(Button, { variant: 'danger', size: 'sm', onClick: clearRecentNotifications },
 ...
 React.createElement('div', { className: 'mb-4 flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl' },
 React.createElement('div', { className: 'w-2 h-2 bg-green-500 rounded-full flex-shrink-0' }),
 React.createElement('p', { className: 'text-xs text-slate-500 ' },
 React.createElement('span', { className: 'font-medium text-slate-600 ' }, 'MongoDB Notifications: '),
 ...
 ? React.createElement('div', { className: 'space-y-2' },
 [1, 2, 3, 4, 5].map(i =>
 React.createElement('div', { key: i, className: 'bg-white rounded-xl p-3 flex gap-3 border border-slate-100 ' },
 React.createElement('div', { className: 'shimmer-bg w-8 h-8 rounded-lg flex-shrink-0' }),
 ...
 : React.createElement(Card, { className: 'p-1 divide-y divide-slate-100 ' },
 filtered.map(notif =>
...
function ProfilePage() {
 ...
 React.createElement('div', { className: 'absolute -bottom-1 -right-1 w-6 h-6 bg-green-500 rounded-full border-2 border-white ', 'aria-label': 'Online status' })
 ),
 React.createElement('div', { className: 'flex-1' },
 React.createElement('h2', { className: 'text-xl font-bold text-[#0F172A]' }, profileLoading ? 'Loading profile...' : profile.name),
 React.createElement('p', { className: 'text-sm text-slate-500 mt-0.5' }, `${String(profile.role || '').toUpperCase()} · ${profile.department || 'N/A'}`),
 ...
 React.createElement('span', { key: tag, className: 'text-xs px-2 py-0.5 bg-teal-50 text-teal-700 rounded-full border border-teal-200 ' }, tag)
 )
 )
 )
 )
 ...
 React.createElement('div', { key: k, className: 'bg-white rounded-xl p-3 border border-slate-200' },
 React.createElement('p', { className: 'text-xs text-slate-500 font-medium mb-0.5' }, k),
 React.createElement('p', { className: 'text-sm font-semibold text-[#0F172A] break-all' }, v)
 )
 )
 )
 ),
