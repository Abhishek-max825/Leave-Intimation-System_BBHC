...
function ToastItem({ toast, onRemove }) {
 const configs = {
 success: { icon: 'CheckCircle', bg: 'bg-white ', border: 'border-green-500', icon_color: 'text-green-500', title: 'Success' },
 error: { icon: 'XCircle', bg: 'bg-white ', border: 'border-red-500', icon_color: 'text-red-500', title: 'Error' },
 warning: { icon: 'AlertTriangle', bg: 'bg-white ', border: 'border-amber-500', icon_color: 'text-amber-500', title: 'Warning' },
 info: { icon: 'Info', bg: 'bg-white ', border: 'border-teal-500', icon_color: 'text-teal-500', title: 'Info' },
 };
 const c = configs[toast.type] || configs.info;

 return React.createElement('div', {
 className: cn('toast-enter flex items-start gap-3 p-4 rounded-xl shadow-lg border-l-4 ', c.bg, c.border),
 style: { animation: 'slideInRight 0.3s ease-out' }
 },
 React.createElement(Icon, { name: c.icon, size: 20, className: c.icon_color }),
 React.createElement('div', { className: 'flex-1 min-w-0' },
 React.createElement('p', { className: 'text-sm font-semibold text-slate-800 ' }, c.title),
 React.createElement('p', { className: 'text-xs text-slate-500 mt-0.5 leading-relaxed' }, toast.msg)
 ),
 React.createElement('button', {
 onClick: () => onRemove(toast.id),
 className: 'p-0.5 rounded text-slate-400 hover:text-slate-600 smooth-transition flex-shrink-0',
 },
...
function SkeletonCard({ className = '' }) {
 return React.createElement('div', { className: cn('bg-white rounded-2xl p-5 shadow-card border border-slate-100 ', className) },
...
function SkeletonTable() {
 ...
 [1, 2, 3, 4].map(i =>
 React.createElement('div', { key: i, className: 'bg-white rounded-xl p-4 border border-slate-100 flex gap-4 items-center' },
...
function Spinner({ size = 'md', className = '' }) {
 ...
 return React.createElement('div', {
 className: cn('border-2 border-slate-200 border-t-teal-500 rounded-full animate-spin', sizes[size], className),
 role: 'status',
 'aria-label': 'Loading...',
 });
}
...
function LoadingOverlay({ message = 'Loading...' }) {
 ...
 React.createElement('p', { className: 'text-sm text-slate-500 ' }, message)
 );
}
...
function EmptyState({ icon = 'FileText', title, message, actionLabel, onAction }) {
 ...
 React.createElement('div', { className: 'w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center' },
 React.createElement(Icon, { name: icon, size: 28, className: 'text-slate-400 ' })
 ),
 React.createElement('div', null,
 React.createElement('h3', { className: 'text-base font-semibold text-slate-700 mb-1' }, title),
 React.createElement('p', { className: 'text-sm text-slate-500 max-w-xs' }, message)
 ),
...
function ErrorState({ message = 'Something went wrong.', onRetry }) {
 return React.createElement('div', { className: 'flex flex-col items-center justify-center gap-4 py-16 text-center' },
 React.createElement('div', { className: 'w-16 h-16 bg-red-50 rounded-2xl flex items-center justify-center' },
 React.createElement(Icon, { name: 'AlertCircle', size: 28, className: 'text-red-400' })
 ),
 React.createElement('div', null,
 React.createElement('h3', { className: 'text-base font-semibold text-slate-700 mb-1' }, 'Failed to load'),
 React.createElement('p', { className: 'text-sm text-slate-500 max-w-xs' }, message)
 ),
 onRetry && React.createElement('button', {
 onClick: onRetry,
 className: 'btn-press px-5 py-2.5 border border-slate-200 hover:bg-slate-50 text-sm font-medium rounded-xl smooth-transition flex items-center gap-2 text-slate-700 '
 },
...
function InputField({ label, id, error, helperText, className = '', required, ...props }) {
 ...
 helperText && !error && React.createElement('p', { id: `${id}-hint`, className: 'text-xs text-slate-500 ' }, helperText)
 );
}
...
function SelectField({ label, id, error, helperText, className = '', children, required, ...props }) {
 ...
 helperText && !error && React.createElement('p', { className: 'text-xs text-slate-500 ' }, helperText)
 );
}
...
function NotificationItem({ notif, onMarkRead, compact = false }) {
 ...
 compact ? 'p-2 hover:bg-slate-50 ' : 'p-3 hover:bg-slate-50 ',
 !notif.read && 'bg-teal-50/60 '
 ),
 ...
 React.createElement('p', { className: cn('text-sm font-medium text-slate-800 leading-tight', !notif.read && 'font-semibold') }, notif.title),
 React.createElement('span', { className: 'text-xs text-slate-400 flex-shrink-0' }, formatRelativeTime(notif.timestamp))
 ),
 React.createElement('p', { className: 'text-xs text-slate-500 mt-0.5 leading-relaxed line-clamp-2' }, notif.message)
 ),
...
// SIDEBAR COMPONENT
// ============================================================
function Sidebar({ currentPage, onNavigate, student, unreadCount, isOpen, onClose }) {
...
// DESKTOP HEADER
// ============================================================
function DesktopHeader({ currentPage, student, unreadCount, onNavigate }) {
 const pageNames = {
 dashboard: 'Dashboard',
 apply: 'Apply Leave',
 'faculty-apply': 'Apply Leave',
 ...
 return React.createElement('header', { className: 'hidden lg:flex sticky top-0 z-10 h-16 bg-white border-b border-[#E2E8F0] px-6 items-center justify-between', 'aria-label': 'Desktop header' },
 React.createElement('div', { className: 'flex items-center h-full' },
 React.createElement('h2', { className: 'text-[14px] font-medium text-[#0F172A] leading-none' }, pageNames[currentPage] || 'Dashboard')
 ),
 React.createElement('div', { className: 'flex items-center gap-3' },
...
// APP LAYOUT WRAPPER
// ============================================================
function AppLayout({ children, currentPage, onNavigate, student, unreadCount }) {
 ...
 React.createElement(DesktopHeader, { currentPage, student, unreadCount, onNavigate }),
...
function LeaveCard({ leave, onViewDetails }) {
 ...
 React.createElement('div', { className: 'w-9 h-9 bg-teal-50 rounded-xl flex items-center justify-center flex-shrink-0' },
 React.createElement(Icon, { name: leave.typeKey === 'sick' ? 'Stethoscope' : leave.typeKey === 'emergency' ? 'Siren' : 'Calendar', size: 17, className: 'text-teal-600 ' })
 ),
 React.createElement('div', { className: 'flex-1 min-w-0' },
 React.createElement('p', { className: 'text-sm font-semibold text-slate-800 truncate' }, leave.type),
 React.createElement('p', { className: 'text-xs text-slate-500 line-clamp-1 mt-0.5' }, leave.reason)
 )
 ),
 React.createElement(StatusBadge, { status: leave.status })
 ),
 React.createElement('div', { className: 'flex items-center gap-4 text-xs text-slate-500 flex-wrap' },
 ...
 React.createElement('span', { className: 'text-purple-600 font-medium' }, `${leave.aiPrediction}% AI`)
 )
 ),
 leave.facultyName && React.createElement('div', { className: 'mt-2 text-xs text-slate-500 ' },
 `Sent to faculty: ${leave.facultyName}`
 ),
 leave.facultyRemark && React.createElement('div', { className: 'mt-3 px-2.5 py-1.5 bg-slate-50 rounded-lg text-xs text-slate-500 flex items-start gap-1.5' },
...
function LeaveDetailModal({ leave, onClose }) {
 ...
 React.createElement('div', { className: 'relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-slide-in-up' },
 React.createElement('div', { className: 'sticky top-0 bg-white px-5 py-4 border-b border-slate-100 flex items-center justify-between rounded-t-2xl z-10' },
 React.createElement('div', null,
 React.createElement('h2', { id: 'modal-title', className: 'text-base font-semibold text-[#0F172A]' }, 'Leave Request Details'),
 React.createElement('p', { className: 'text-xs text-slate-400 mt-0.5' }, leave.id)
 ),
 React.createElement('button', { onClick: onClose, className: 'p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 smooth-transition', 'aria-label': 'Close modal' },
 React.createElement(Icon, { name: 'X', size: 18 })
 )
 ),
 React.createElement('div', { className: 'px-5 py-4 space-y-4' },
 React.createElement('div', { className: 'flex items-center justify-between' },
 React.createElement('span', { className: 'text-sm font-medium text-[#0F172A]' }, leave.type),
 ...
 React.createElement('p', { className: 'text-xs text-slate-500 font-medium mb-1' }, k),
 React.createElement('p', { className: 'text-sm font-semibold text-[#0F172A]' }, v)
 )
 )
 ),
 React.createElement('div', { className: 'space-y-2' },
 React.createElement('p', { className: 'text-sm font-medium text-[#0F172A]' }, 'Reason for Leave'),
 React.createElement('div', { className: 'bg-white rounded-xl p-3 border border-slate-200' },
 React.createElement('p', { className: 'text-sm text-[#374151] leading-relaxed' }, leave.reason)
 )
 ),
 leave.facultyRemark && React.createElement('div', { className: 'space-y-2' },
 React.createElement('p', { className: 'text-sm font-medium text-slate-700 flex items-center gap-1.5' },
 React.createElement(Icon, { name: 'MessageCircle', size: 14 }),
 'Faculty Remark'
 ),
 React.createElement('div', { className: cn('rounded-xl p-3 border', leave.status === 'approved' ? 'bg-green-50 border-green-200 ' : leave.status === 'rejected' ? 'bg-red-50 border-red-200 ' : 'bg-slate-50 border-slate-200 ') },
 React.createElement('p', { className: 'text-sm leading-relaxed text-slate-700 ' }, leave.facultyRemark)
 )
 ),
